...
class PyramidWorkingResearchSystem:
    ...
    def __init__(self, hf_token: str, model_id: str = "damo-vilab/text-to-video-ms-1.7b", output_base_dir: str = "pyramid_working_research_output"):
    ...
    def _wait_for_queue(self, response_json: Dict[str, Any], max_wait_seconds: int = 600) -> bool:
        """Wait according to HF queue info when the model is loading or busy."""
        start = time.time()
        while True:
            if time.time() - start > max_wait_seconds:
                return False
            estimated = response_json.get("estimated_time") or 10
            sleep_for = min(max(1, int(estimated)), 30)
            print(f"⏳ Model loading/busy. Waiting {sleep_for}s (estimated {estimated}s)...")
            time.sleep(sleep_for)
            # a small ping to see if model ready
            ping = requests.post(self.api_url, headers=self._headers(accept_video=False), json={"inputs": "ping"}, timeout=30)
            if ping.status_code == 200 and ping.headers.get('content-type','').startswith('application/octet-stream'):
                return True
            try:
                pj = ping.json()
            except Exception:
                pj = {}
            if ping.status_code == 200 and isinstance(pj, dict) and not pj.get("error"):
                # may still be JSON; continue waiting
                continue
            if ping.status_code == 503:
                response_json = pj if isinstance(pj, dict) else response_json
                continue
            # For other codes, continue until timeout
    ...
    # Reuse the detection/report structure from the HeyGen system by simple import
    # to avoid code duplication if available in same package. Fallback: lightweight in-file versions.
